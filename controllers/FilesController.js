...
export default class FilesController {
  static async  postUpload(req, res) {
       ...
       let { name, type, parentId, isPublic, data } = req.body;
        ...
        if (type === "folder") {
            const newFile = await dbClient.db.collection('files').insertOne({name, type, parentId,  userId: user._id});
            res.status(201).json({name, type, userId, isPublic, parentId, id: newFile.insertedId});
            ...
            const newFile = await dbClient.db.collection('files').insertOne({name, type, parentId, isPublic, userId: user._id, localPath: fileLocalPath});
            res.status(201).json({id: newFile.insertedId, userId, name, type, isPublic, parentId});
         }
